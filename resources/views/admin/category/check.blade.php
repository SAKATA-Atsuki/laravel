...
<body id="admin">
    ...
    <div class="admin-category-register">
        ...
        <div class="admin-category-register-content">
            ...
            <span>{{ $session_category_register['category'] }}</span>
        </div>
        <br>
        <div class="admin-category-register-content">
            <span class="admin-category-register-content-left">商品小カテゴリ</span>
            <span>{{ $session_category_register['subcategory1'] }}</span>
        </div>
        @for ($i = 2; $i <= 10; $i++)
            @if ($session_category_register['subcategory' . $i] != '')
                ...
                <div class="admin-category-register-content">
                    ...
                    <span>{{ $session_category_register['subcategory' . $i] }}</span>
                </div>    
            @endif
        @endfor
        <form action="{{ route('admin.category.register.store') }}" method="POST">
            @csrf
